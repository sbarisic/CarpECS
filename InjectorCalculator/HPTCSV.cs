...
namespace InjectorCalculator {
	struct CSVPoint {
		...
		public CSVPoint(string RPM, string CylAirmass, string MAT, string AFR, string InjMS, string AccelPedal) {
	...
	class HPTCSV {
		...
		public static CSVPoint[] ParseEntriesHPT(string CSVFile) {
			...
			for (int i = 0; i < DataLines.Length; i++) {
				CSVPoint Point = new CSVPoint(DataLines[i][RPMIdx], DataLines[i][Airmass], DataLines[i][MAT], DataLines[i][AFR], DataLines[i][InjMS], DataLines[i][AccelPedal]);
